...
class UserAgent(BaseAgent):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        logger.info(f"UserAgent-----------__init__:{name}")
        logger.info(f"UserAgent-----------__init__:{sys_prompt}")
    ...
    async def a_generate_rsp(
        ...
        logger.info("UserAgent::a_generate_rsp begin!")
        ...
        #--------------------------------------------------------------------------------------------------------
        num_llm_calls_available = kwargs.pop('num_llm_calls', 1)
        logger.info(f"UserAgent::a_generate_rsp  ---- num_llm_calls_available:{num_llm_calls_available}!")
    ...
    async def get_llm_reply(
        ...
        **kwargs: Any
        ...
        logger.info("UserAgent::get_llm_reply ---- begin!")
        #--------------------------------------------------------------------------------------------------------
        # 调用大模型次数默认设置为2次，第一次调用llm，可能因为模型返回了tool call，所以在调用了 tool后，还需要调用一次获取最终结果。
        # 多次调用大模型返回多次都是tool继续调用的这种情况暂不考虑，这种情况直接修改num_llm_calls 参数即可
        #--------------------------------------------------------------------------------------------------------
        while num_llm_calls_available > 0:
            ...
            logger.info(f"UserAgent::get_llm_reply ---- num_llm_calls_available{num_llm_calls_available}!")
            ...
            if isinstance(responses, AsyncIterator):
                ...
                if collected_responses:
                    ...
                    logger.info(f"UserAgent::get_llm_reply ---fff--- 【has_tool_call:{has_tool_call}】 【tool_call_msg:{tool_call_msg}】 【tool_results:{tool_results}】 !")
                    if has_tool_call:
                        ...
                        ####################################################################
                        logger.info(f"UserAgent::get_llm_reply ---fff--- combined_results:{combined_results}")
                        ...
                        if (len(tool_results) == 1 
                            ...
                            logger.info(f"UserAgent::get_llm_reply --- Switching to {tool_results[0].extra['switch_mode_to']}")
                            # 不在调用 llm
